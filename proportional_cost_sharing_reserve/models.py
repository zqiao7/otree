...
doc = """
Proportional cost sharing with reservation.
"""
...
class Group(otree.models.BaseGroup):
    ...
    def set_payoffs(self):
        ...
        contrib_1 = [ x for x in contrib if x >= 22 ]
        total_contrib = sum(contrib_1)
        self.provision_success = total_contrib > Constants.cost
        
        if self.provision_success:
            for p in self.get_players():
                p.member = int(p.contribution) >= 22
                p.share = int(int(p.contribution) >= 22)*int(p.contribution)/total_contrib
                p.payoff = int(int(p.contribution) >= 22)*(p.private_value - Constants.cost * p.share)
        else:
            for p in self.get_players():
                p.member = 1>2
                p.share = 0
