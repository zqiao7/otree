	...
	def set_payoffs(self):
		...
		if self.provision_success:
			for p in self.get_players():
				p.member = p.contribution >= 22
				p.share = int(p.contribution >= 22)*p.contribution/total_contrib
				p.payoff = int(p.contribution >= 22)*(p.private_value - Constants.cost * p.share)
		else:
			for p in self.get_players():
				p.member = 1>2
				p.share = 0
